(ns isaac.component.supervisor
  "Generic supervision for long-lived server subsystems (isaac-royn). Watches
   every started Component that implements protocol/Supervised: when a worker's
   liveness probe reports dead, it logs at ERROR, restarts the component
   ...
   Each supervised component is restarted independently — a crash in one never
...
(defn health
  "Snapshot of per-subsystem supervision health:
   {component-id {:status :up|:restarting|:down :restarts n :last-error s}}."
  []
  @health*)
  ...
  (let [restarts' (inc (or restarts 0))]
    (if (> restarts' default-max-restarts)
      (do
        ...
        (log/error :component/supervision-exhausted :component (name id) :restarts restarts'))
      (do
        (log/error :component/died :component (name id) :restarts restarts')
        (let [err  (try (restart! instance) nil
                        (catch Throwable t (.getMessage t)))
              next (str (.plusMillis now (backoff-ms restarts')))]
          (when err
            (log/error :component/restart-failed :component (name id) :restarts restarts' :error err))
...
(defn- supervise-component! [{:keys [id instance]} now]
...
(defn supervise-once!
  "One monitoring pass over the supervised subset of `started-components` (each
   `{:id :instance}`). Never throws — a failure supervising one component must not
   stop the others."
  [started-components now]
  (doseq [{:keys [id instance] :as entry} started-components]
    (when (protocol/supervised? instance)
      (try
        (supervise-component! entry now)
        (catch Throwable t
          (log/error :component/supervise-failed :component (name id) :error (.getMessage t)))))))
...
(defn start!
  "Start the background supervision loop. `components-fn` returns the current
   started-component entries (`{:id :instance}`); the loop polls every poll-ms.
   The loop itself is crash-proof: any error in a pass is logged and the loop
   continues."
  ([components-fn] (start! components-fn default-poll-ms))
  ([components-fn poll-ms]
   (when-not @supervisor*
     (let [running? (atom true)
           thread   (Thread.
                      ...
                      (fn []
                        (while @running?
                          (try
                            (supervise-once! (components-fn) (Instant/now))
                            (catch Throwable t
                              (log/error :component/supervisor-loop-error :error (.getMessage t))))
                          (try (Thread/sleep (long poll-ms))
                               (catch InterruptedException _ nil))))
                      "isaac-component-supervisor")]
