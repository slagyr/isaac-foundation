(ns isaac.component.supervisor-spec
  (:require
    [isaac.logger :as log]
    [isaac.component.protocol :as protocol]
    [isaac.component.supervisor :as sut]
...
(defn- fake-component [alive starts]
  (reify
    protocol/Component
...
(describe "component supervisor"
  ...
  (it "leaves a healthy supervised component untouched"
    (let [starts (atom 0)
          svc    (fake-component (atom true) starts)]
      (sut/supervise-once! [{:id :worker :instance svc}] t0)
      (should= 0 @starts)
      (should= :up (:status (get (sut/health) :worker)))))

  (it "ignores components that are not Supervised"
    (let [svc (reify protocol/Component (start [_]) (stop [_]))]
      (sut/supervise-once! [{:id :plain :instance svc}] t0)
      (should= {} (sut/health))))

  (it "restarts a supervised component that dies and logs at ERROR"
    (let [alive  (atom true)
          starts (atom 0)
          svc    (fake-component alive starts)
      ...
      (should-contain :component/died (events))))

  (it "resets the breaker after the component recovers and stays healthy"
  ...
  (it "waits out the backoff before restarting again"
    (let [starts (atom 0)
          svc    (fake-component (atom false) starts)
  ...
  (it "gives up after the max restarts and marks the subsystem down"
    (let [starts (atom 0)
      ...
      (should-contain :component/supervision-exhausted (events))))

  (it "restarts only the dead component, leaving healthy siblings untouched (isolation)"
    (let [dead-starts (atom 0)
          ok-starts   (atom 0)
          entries [{:id :dead :instance (fake-component (atom false) dead-starts)}
                   {:id :ok   :instance (fake-component (atom true)  ok-starts)}]]
      (sut/supervise-once! entries t0)
  ...
  (it "keeps supervising the others when one component's probe throws"
    (let [ok-starts (atom 0)
          ...
          entries [{:id :boom :instance boom}
                   {:id :dead :instance (fake-component (atom false) ok-starts)}]]
      (sut/supervise-once! entries t0)
      (should-contain :component/supervise-failed (events))
      (should= 1 @ok-starts))))
