...
(defn- discover-local! [ids]
  (with-redefs [isaac.module.classpath/invoke-add-deps! (fn [_])
                isaac.module.discovery/manifest-resource local-manifest-path]
...
(describe "module loader discovery"
  ...
  (describe "foundation-index"
    ...
    (it "reads the foundation manifest only once"
      (let [resource-calls (atom 0)
            read-calls     (atom 0)]
        (with-redefs-fn {#'isaac.module.discovery/manifest-resource (fn [_]
          ...
          #(do
             (should= {:isaac.foundation {:coord {}
                                          ...
                                          :path nil}}
                      (discovery/foundation-index))
             (should= (discovery/foundation-index) (discovery/foundation-index))))
  ...
  (describe "discover!"
    ...
    (it "includes builtin manifests even when :modules is absent"
      (let [{:keys [index errors]} (discovery/discover! {} ctx)]
        (should= [] errors)
    ...
    (it "discovers local/root manifests via classpath loading"
      (let [cwd         (System/getProperty "user.dir")
            ...
            result      (discovery/discover! {:modules {:marigold.cli.greeter {:local/root module-root}}}
                                      (assoc ctx :cwd cwd))]
    ...
    (it "invalidates builtin-index cache when a module dep is dynamically loaded"
      (let [invalidated? (atom false)
        ...
        (with-redefs [discovery/invalidate-builtin-index! (fn [] (reset! invalidated? true))]
          (discovery/discover! {:modules {:marigold.cli.greeter {:local/root "modules/marigold.cli.greeter"}}}
                         (assoc ctx :cwd cwd))
          (should @invalidated?))))

    (it "adds an error when a local/root path is not found"
      (let [{:keys [index errors]} (discovery/discover! {:modules {:isaac.comm.ghost {:local/root "/state/.isaac/modules/isaac.comm.ghost"}}} ctx)]
    ...
    (it "reads a local/root manifest directly when no deps.edn is present"
      (let [root (mod-root :isaac.comm.broken)]
        ...
        (let [calls (atom [])]
          (with-redefs [isaac.module.classpath/invoke-add-deps! (fn [_])]
            (let [{:keys [index errors]} (discovery/discover! {:modules {:isaac.comm.broken {:local/root root}}} ctx)]
              (should= [] errors)
    ...
    (it "uses the installed runtime fs for local manifest discovery"
      (let [mem  (fs/mem-fs)
        ...
        (nexus/-with-nexus {:fs mem}
          (let [{:keys [index errors]} (discovery/discover! {:modules {:isaac.comm.runtime {:local/root root}}} ctx)]
            (should= [] errors)
    ...
    (it "resolves overlapping explicit and manifest :deps modules in one add-deps pass"
      ...
      (let [calls            (atom [])
        ...
        (with-redefs [isaac.module.discovery/manifest-resource
                      (fn [id]
                        (when (contains? @classpath-ready? id)
                          (str (mod-root id) "/resources/isaac-manifest.edn")))
                      isaac.module.classpath/invoke-add-deps!
                      (fn [deps-map]
          ...
          (let [{:keys [errors index]}
                (discovery/discover! {:modules {:mod.server (mod-coord :mod.server)
                      ...
                      (fn [deps-map]
                        ...
                        (doseq [[lib-sym _] deps-map]
                          (when-let [ns (namespace lib-sym)]
                            (swap! classpath-ready? conj (keyword ns)))))]
          (let [{:keys [errors index]}
                (discovery/discover! {:modules {:mod.client (mod-coord :mod.client)}} ctx)]
            (should= [] errors)
            (should= :mod.server (get-in index [:mod.server :manifest :id]))
            (should= 1 (count (filter #(contains? % 'mod.server/mod.server) @calls)))))))
    ...
    (it "adds module deps only once per coordinate across repeated discovery"
      ...
      (let [calls            (atom [])
        ...
        (with-redefs [isaac.module.discovery/manifest-resource (fn [id]
                                                              (when (and @classpath-ready?
                                                                         (= id :isaac.comm.pigeon))
                                                                (str (mod-root :isaac.comm.pigeon) "/resources/isaac-manifest.edn")))
                      isaac.module.classpath/invoke-add-deps!   (fn [deps-map]
                                                              (swap! calls conj deps-map)
                                                              (reset! classpath-ready? true))]
          (let [first-result  (discovery/discover! {:modules {:isaac.comm.pigeon (mod-coord :isaac.comm.pigeon)}} ctx)
                second-result (discovery/discover! {:modules {:isaac.comm.pigeon (mod-coord :isaac.comm.pigeon)}} ctx)
    ...
    (it "discovers builtin classpath manifests and ignores unflagged manifests"
      (let [real-resource-urls @#'isaac.module.discovery/resource-urls]
        (with-redefs [isaac.module.discovery/resource-urls
